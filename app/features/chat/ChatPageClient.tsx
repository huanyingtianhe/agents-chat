...
export function ChatPageClient() {
  ...
  async function loadChat(chatId: string) { if (chatId !== currentChatId) { setOpenChatMenuId(null); shouldStickToBottomRef.current = true; setShowScrollToBottom(false); await runtimeLoadChat(chatId); if (currentChatIdRef.current !== chatId) return; requestAnimationFrame(() => { const el = chatContainerRef.current; if (el) el.scrollTop = el.scrollHeight; }); } setOpenChatMenuId(null); if (mobile.isMobileLayout) mobile.close(); else setShowChatsPanel(false); }
