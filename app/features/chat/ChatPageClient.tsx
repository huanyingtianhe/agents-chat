...
export function ChatPageClient() {
  ...
  const fileWorkspace = useFileWorkspaceState({ agents, agentsLoading, mounted, schedulerAgentId: SCHEDULER_AGENT_ID, onBeforeTabChange: handleBeforeFileTabChange, onFileOpened: ({ agentId, filePath, restoreScrollTop }) => {
    fileCommentsControllerRef.current?.resetForFileOpen(agentId, filePath, restoreScrollTop);
  ...
  async function loadChat(chatId: string) { if (chatId === currentChatId) { setOpenChatMenuId(null); if (mobile.isMobileLayout) mobile.close(); else setShowChatsPanel(false); return; } setOpenChatMenuId(null); shouldStickToBottomRef.current = true; setShowScrollToBottom(false); await runtimeLoadChat(chatId); if (mobile.isMobileLayout) mobile.close(); requestAnimationFrame(() => { const el = chatContainerRef.current; if (el) el.scrollTop = el.scrollHeight; }); }
