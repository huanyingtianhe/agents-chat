...
export function ChatPageClient() {
  ...
  const { messages, chatHistory, currentChatId, activeSidebarChatId, chatName, runVersion, shareDialog, expandedMessages, orchestrationMode, setChatHistory, setChatName, setCurrentChatId, setActiveSidebarChatId, setShareDialog, setExpandedMessages, setOrchestrationMode, currentChatIdRef, sessionRunsRef, currentAgentSessionsRef, inputHistoryRef, addMessage, updateMessage, notifyRunStateChanged, dispatchToAgent, saveCurrentChatToHistory, clearChatMessages, shareCurrentChat, handleStop, retryFailedSend, answerAgentUserRequest, dismissAgentUserRequest, fileCommentCallbacksRef, panelCallbacksRef, loadChat: runtimeLoadChat, createNewChat: runtimeCreateNewChat, renameChatById: runtimeRenameChatById, deleteChatById: runtimeDeleteChatById, handleSend: runtimeHandleSend, loadChatIntoCache, getChatSidebarStatus } = runtime;
  ...
  const targetControls = <ComposerTargetControls mentionedAgentIds={mentionedAgentIds} orchestrationEnabled={mentionedAgentIds.length > 1} orchestrationMode={orchestrationMode} effectiveComposerAgentId={effectiveComposerAgentId} rememberedComposerAgentId={rememberedComposerAgentId} currentChatId={currentChatId} getAgentModels={(agentId) => agents.find((agent) => agent.id === agentId)?.models || []} getSelectedModelIdForAgent={getSelectedModelIdForAgent} openModelMenuAgentId={agentPanelState.openModelMenuAgentId} setOpenModelMenuAgentId={agentPanelState.setOpenModelMenuAgentId} modelMenuRefs={agentPanelState.modelMenuRefs} setSelectedModelForAgent={registry.setSelectedModelForAgent} clearLastUsedAgent={() => registry.clearLastUsedAgent(currentChatId || undefined)} setOrchestrationMode={setOrchestrationMode} />;
