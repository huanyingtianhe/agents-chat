...
export function ChatPageClient() {
  ...
  function selectSlashCommand(command: { name: string }) { setInputProgrammatic(`/${command.name} `); setSlashSelectedIndex(0); composerRef.current?.focus(); }
  ...
  function handleComposerKeyDown(e: KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (filteredSlashCommands.length > 0) { if (e.key === 'ArrowDown') { e.preventDefault(); setSlashSelectedIndex((p) => (p + 1) % filteredSlashCommands.length); return; } if (e.key === 'ArrowUp') { e.preventDefault(); setSlashSelectedIndex((p) => (p - 1 + filteredSlashCommands.length) % filteredSlashCommands.length); return; } if (e.key === 'Tab') { e.preventDefault(); selectSlashCommand(filteredSlashCommands[slashSelectedIndex] || filteredSlashCommands[0]); return; } if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); selectSlashCommand(filteredSlashCommands[slashSelectedIndex] || filteredSlashCommands[0]); return; } if (e.key === 'Escape') { e.preventDefault(); setInputProgrammatic(''); return; } }
