...
export function ChatPageClient() {
  ...
  async function invokeSlashCommand(command: { name: string }) { setInputProgrammatic(`/${command.name}`); setSlashSelectedIndex(0); await handleSend(); }
  ...
  function handleComposerKeyDown(e: KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (filteredSlashCommands.length > 0) { if (e.key === 'ArrowDown') { e.preventDefault(); setSlashSelectedIndex((p) => (p + 1) % filteredSlashCommands.length); return; } if (e.key === 'ArrowUp') { e.preventDefault(); setSlashSelectedIndex((p) => (p - 1 + filteredSlashCommands.length) % filteredSlashCommands.length); return; } if (e.key === 'Tab') { e.preventDefault(); insertSlashCommand(filteredSlashCommands[slashSelectedIndex] || filteredSlashCommands[0]); return; } if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void invokeSlashCommand(filteredSlashCommands[slashSelectedIndex] || filteredSlashCommands[0]); return; } if (e.key === 'Escape') { e.preventDefault(); setInputProgrammatic(''); return; } }
