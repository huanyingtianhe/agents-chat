...
assert.match(fileHelpersSource, /function isHtmlFile/, 'Files UI should detect HTML files separately from markdown');
assert.match(fileEditorSource, /sandbox=""/, 'Rendered HTML preview should use a sandboxed iframe');
assert.match(fileEditorSource, /srcDoc=\{workspace\.mdFileContent\}/, 'Rendered HTML preview should feed the selected file content to the iframe');
assert.match(fileEditorSource, /mdPreviewBadge/, 'HTML preview should be labeled as rendered HTML in the toolbar');
