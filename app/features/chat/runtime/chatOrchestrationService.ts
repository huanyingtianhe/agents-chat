  ...
  async function maybeAdvanceOrchestration(orchestrationId: string) {
    ...
    if (state.mode === 'auto') {
      ...
      if (missing.length > 0) {
        ctx.addMessage({
          type: 'system',
          content: `⚠️ Auto mode: scheduler's plan did not include mentioned agent(s): ${missing.join(', ')}. Each mentioned agent should contribute. Consider retrying or use Workflow mode for full control.`,
        }, orchestrationChatId);
      }
