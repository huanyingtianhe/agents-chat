...
test('composer model picker saves the selected model as the agent default', async ({ page }) => {
  let savedDefaultModelId = 'claude-sonnet-4.6';
  ...
  const alphaAgent = () => ({
    ...
    defaultModelId: savedDefaultModelId,
  });
  ...
  await page.route('**/api/acp', async (route) => {
    ...
    if (body?.action === 'update-agent-config') {
      updateRequests.push(body);
      savedDefaultModelId = body.updates.defaultModelId;
      await route.fulfill({ contentType: 'application/json', body: JSON.stringify({ ok: true, agent: alphaAgent(), restarted: false }) });
      return;
    }
    if (body?.action === 'send') {
  ...
  await composerModelSelect.selectOption('gpt-5.2');
  await expect(composerModelSelect).toHaveValue('gpt-5.2');

  await expect.poll(() => updateRequests.map((request) => request.updates.defaultModelId)).toEqual(['gpt-5.2']);
