...
export function createOrchestrationHandlers(ctx: OrchestrationContext) {
  ...
  async function runAutoOrchestration(orchestrationId: string, agentIds: string[], task: string, originalText: string, chatId: string, promptAttachments: ChatAttachment[] = []) {
    ...
    const planPrompt = [
      ...
      'The agents listed above are the POOL you can use; you do NOT have to use all of them.',
      'Pick the minimum set of nodes that accomplishes the user request. Prefer one node when one is enough.',
      'Only add multiple nodes when the work genuinely splits (e.g. parallel independent tasks, or distinct roles like reviewer + fixer).',
      'When the user assigns explicit roles to specific agents (e.g. "@a reviews, @b fixes"), honor that mapping.',
      'When two nodes can work independently on the same input, give them dependsOn: [] so they run in parallel.',
    ].join('\n');
